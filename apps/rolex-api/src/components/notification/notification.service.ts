...
export class NotificationService {
	notify(arg0: { receiverId: Schema.Types.ObjectId; authorId: Schema.Types.ObjectId; type: NotificationType; group: NotificationGroup; title: string; desc: string; refId: string; }) {
	...
	public async removeAllNotifications(receiverId: ObjectId): Promise<boolean> {
		const result = await this.notificationModel.deleteMany({ receiverId }).exec();
	...
	public async unreadCount(receiverId: ObjectId): Promise<number> {
		console.log('receiverId type:', typeof receiverId, receiverId);
		const exists = await this.notificationModel.findOne({ receiverId });
		...
		return this.createNotification(
			{
				...
				notificationTitle: 'Welcome to Rolex',
